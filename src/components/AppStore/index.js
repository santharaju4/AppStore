...
class AppStore extends Component {
  ...
  render() {
    ...
    const searchResults = appsList.filter(eachValue =>
      eachValue.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
      ...
      <div className="app-container">
        ...
        <ul className="tabs-list">
          {tabsList.map(eachValue => (
            <TabItem
              tabDetails={eachValue}
              key={eachValue.tabId}
              setActiveTabId={this.setActiveTabId}
              isActive={eachValue.tabId === activeTabId}
            />
          ))}
        </ul>
        <ul className="apps-list">
          {filterResult.map(eachValue => (
            <AppItem appItemDetails={eachValue} />
          ))}
        </ul>
      </div>
    )
